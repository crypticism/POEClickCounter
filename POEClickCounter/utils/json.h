// Taken from Microsoft PowerToys https://github.com/microsoft/PowerToys/blob/master/src/common/utils/json.h
	...
	inline std::optional<JsonObject> from_file(std::wstring_view file_name)
	{
		try
		{
			...
			if (file.is_open()) {
				...
				return JsonValue::Parse(winrt::to_hstring(obj_str)).GetObject();
			}
			return std::nullopt;
		}
		catch (...) {
